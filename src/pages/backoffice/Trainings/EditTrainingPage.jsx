...
import { fetchTrainingById } from '../../../api/trainings/fetchTrainingById';
import { editTrainingById } from '../../../api/trainings/editTrainingById';
...
const EditTrainingPage = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        date: new Date(),
        readingDuration: '',
        tags: [], // Will store full tag objects
        content: '',
    });
    ...
    useEffect(() => {
        const fetchTraining = async () => {
            try {
                const response = await fetchTrainingById(uuid, token);
                if (response.status === 200) {
                    ...
                    setFormData({
                        ...
                        readingDuration: training.reading_duration,
                        tags: training.tags,
                        content: training.content,
                    });
                } else {
                    setError(response.data.message || 'Error fetching training.');
                }
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            tags: formData.tags, // Just send array of UUIDs
        ...
        try {
            const response = await editTrainingById(payload, uuid, token);

            if (response.status === 200) {
                setSuccess(true);
            } else {
                setError(response.data.message || 'Error al modificar la capacitación.');
            }
        } catch (err) {
    ...
    const handleCloseSuccessModal = () => {
        setSuccess(false);
        navigate('/backoffice/capacitaciones');
    };
    ...
    return (
        <div className="p-1 w-full max-w-5xl mx-auto">
            <h1 className="text-2xl mb-6 font-bold text-primary">Editar capacitación</h1>
            ...
            {success && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white rounded-lg p-6 w-1/3 text-center">
                        <h2 className="text-xl font-bold text-primary">¡Capacitación modificada!</h2>
