    ...
    <div className="mt-6 md:mt-0 p-6 bg-white rounded-lg shadow-lg font-lato">
      ...
      {detalle.length > 0 && (
        <div className="mb-4 text-sm">
          <p className="font-semibold text-gray-700 mb-1">Detalle inicial</p>
          <ul className="list-disc list-inside">
            {detalle.map((item, idx) => (
              <li key={idx}>
                {item.descripcion}:{" "}
                <span className="font-semibold">{formatPesos(item.valor)}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {calculo_intereses.length > 0 && (
        <div className="mb-4 overflow-x-auto">
          ...
          <table className="min-w-full text-xs border border-gray-200">
            ...
            <tbody>
              {calculo_intereses.map((row, idx) => (
                <tr key={idx} className="odd:bg-white even:bg-gray-50">
                  <td className="border px-2 py-1">{row.periodo}</td>
      ...
      <div className="flex flex-col gap-2 text-sm">
        <div className="px-3 py-2 rounded-lg bg-gray-50 border border-gray-200">
          <p className="font-semibold text-gray-600">Interés total</p>
          <p className="text-base font-bold">{formatPesos(interes_total)}</p>
        </div>
        ...
        <div className="w-full md:w-4/5 max-w-5xl">
          ...
          <div
            className={`grid items-start gap-8 ${
              resultado ? "md:grid-cols-2" : "grid-cols-1"
            }`}
          >
